...
Если журнал обновления TLE содержит `401 Unauthorized`, источник требует
учётные данные или больше не допускает анонимную загрузку. Старый TLE при этом
может продолжать копироваться в `satdump_tles.txt`, но корректную привязку нового
пролёта он не гарантирует. Настройте доступ к источнику либо передайте свежий
валидный файл через `--tle_override`; не подменяйте отсутствующую геометрию
нарисованными «примерными» контурами.
