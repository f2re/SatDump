#!/usr/bin/env bash
# Run INSIDE the existing trusted Debian Stretch build rootfs, not on the target.
set -Eeuo pipefail
...
# ldd prints transitive DT_NEEDED dependencies too; keep SONAMEs, not build paths.
