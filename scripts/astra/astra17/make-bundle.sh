...
# Wrapper и smoke-test запускаются из временного пустого CWD, поэтому все
# рабочие пути должны быть абсолютными и не зависеть от каталога запуска.
...
# Инструкция должна ехать вместе с архивом: целевая машина может быть полностью
# изолирована от репозитория и сети.
cat > "${BUNDLE}/README.ru.md" <<'EOF2'
# SatDump 1.2.2 — переносимый бандл для Astra Linux 1.7

Это готовая x86_64-версия: распакуйте архив и запускайте `./satdump-ui` (графический
интерфейс) или `./satdump` (командная строка). В бандл включены ресурсы SatDump,
плагины выбранного профиля и все их **несистемные** runtime-библиотеки. Системные
glibc/loader/NSS намеренно не заменяются: Astra Linux 1.7 предоставляет glibc 2.28,
а перенос glibc вместе с NSS/DNS небезопасен.
...
Для проверки целостности рядом с архивом выполните `sha256sum -c *.tar.gz.sha256`.
После распаковки сверьте все файлы командой `sha256sum -c SHA256SUMS`; быстрый
проверочный запуск — `./satdump version` внутри бандла.

`./install.sh` необязателен: он копирует бандл в `/opt` (или `~/.local/opt`) и создаёт
команды `satdump`/`satdump-ui` в соответствующем `bin`. Без установки достаточно
запускать файлы из распакованной папки.

## Сценарий 1: полное изображение без обрезки

1. Откройте `./satdump-ui`, выберите **Offline processing**.
2. Выберите декодер, например `meteor_m2x_lrpt`, укажите IQ-файл и папку результата,
   затем нажмите Start.
3. В просмотрщике выберите канал или RGB-композит, при необходимости включите
   географическую проекцию и карту.
4. В разделе оформления выберите режим **Full / Presentation**. Он сохраняет весь
   полезный растр и добавляет полные верхнюю и нижнюю информационные панели.
5. Экспортируйте PNG из viewer. Исходная область изображения не масштабируется и не
   обрезается; панели увеличивают высоту итогового PNG.

CLI-вариант для Meteor-M LRPT:

```bash
./satdump meteor_m2x_lrpt baseband /data/input/meteor_pass.cs16 /data/output/meteor_pass
```

Для NOAA APT выберите соответствующий модуль в Offline processing и WAV-запись;
аналоговые, Meteor и NOAA/MetOp плагины включены в desktop-профиль.

## Сценарий 2: компактная подпись (урезанные плашки)

После формирования композита откройте его в viewer и в настройках оформления выберите
**Minimal / Compact**. Этот вариант оставляет те же данные изображения, но выводит
короткую верхнюю и/или нижнюю плашку: спутник, UTC-время, тип композита и станцию.
Используйте его для каталога, рассылки и серийных продуктов. Текст станции меняется в
настройках SatDump (`default_qth_label`); название продукта — в параметрах композита.

Не используйте ручную обрезку PNG для сокращения подписи: выбирайте Minimal, чтобы
сохранить корректные метаданные, легенду и ориентацию.

## Сценарий 3: карта, проекция и легенда

1. Сначала соберите нужный канал/RGB, затем включите географическую проекцию.
2. Добавьте границы, береговую линию и при необходимости города/координатную сетку.
3. Для температурных или классификационных продуктов включите легенду и выберите
   **Presentation** (полная) либо **Minimal** (компактная) плашку.
4. Проверьте ориентацию по карте перед экспортом, особенно для восходящих пролётов.

## Если не запускается

* Нужна Astra Linux 1.7 x86_64 или система с glibc не ниже указанной в
  `ASTRA17-MANIFEST.txt`.
* Запускайте именно `./satdump` и `./satdump-ui`: wrappers сами задают пути к
  ресурсам, плагинам и библиотекам.
* При проблемах с OpenGL обновите драйвер видеокарты/Mesa в ОС; это системный драйвер,
  он не переносится в архив намеренно.
* Журнал CLI: `./satdump version`; для UI запустите `./satdump-ui` из терминала и
  приложите вывод к обращению.
EOF2
